...
switch ($opcion) {
    case 1: //Prestar
        ...
        $resultado->execute();
        $consulta1 = "UPDATE carpeta SET Estado_item_id_estado_item='2' WHERE id_carpeta='$id'";
        $resultado1 = $conexion->prepare($consulta1);
        $resultado1->execute();


        $consulta2 = "SELECT id_folio FROM folio WHERE Carpeta_id_carpeta = $id";
        ...
        foreach ($data2 as $dat2) {
            ...
            $consulta3 = "UPDATE folio SET Estado_item_id_estado_item = '2' WHERE id_folio='$cod_folio'";
            $resultado3 = $conexion->prepare($consulta3);
            $resultado3->execute();
        }

        $consulta4 = "SELECT codigo_carpeta AS Carpeta, serial_caja As Codigo1, descripcion_caja AS Codigo2, nombre_bodega AS Bodega, telefono_bodega, descripcion_estante AS Estante, descripcion_cara AS Cara, descripcion_modulo AS Modulo,
			...
			FROM bodega, estante, cara, modulo, piso, entrepano, ubicacion_caja, cajas, carpeta
        ...
        foreach ($data4 as $dat4) {
            $carpeta                = $dat4['Carpeta'];
            $serial_caja            = $dat4['Codigo1'];
            $descripcion_caja        = $dat4['Codigo2'];
            $nombre_bodega            = $dat4['Bodega'];
            $telefono_bodega        = $dat4['telefono_bodega'];
            $descripcion_estante    = $dat4['Estante'];
            $descripcion_cara        = $dat4['Cara'];
            $descripcion_modulo        = $dat4['Modulo'];
            $descripcion_piso        = $dat4['Piso'];
            $descripcion_entrepano    = $dat4['Entrepano'];
            $ubicacion_X            = $dat4['Ubicacion_X'];
            $ubicacion_Y            = $dat4['piso_Y'];
            $ubicacion_Z            = $dat4['fondo_Z'];
        }

        $consulta5 = "SELECT razon_social_cliente As nombre FROM clientes WHERE id_cliente = $cliente";
        ...
        foreach ($data5 as $dat5) {
            $nombre_cliente = $dat5['nombre'];
        }

        $to = "$email_usuario,wms@siglo21.com.co,gestor@docsiglo21.com.co";
        $subject = "Nueva solicitud de prestamo de Carpetas";
        $txt = "<div style='background-color: rgb(255, 255, 255); padding: 10px; border: 3px black double; border-radius: 15px 15px 0px 0px;'>
					...
					<h1 style='color: rgb(0, 0, 0);'> Nueva solicitud de prestamo de Carpetas</h1>
				</div>
